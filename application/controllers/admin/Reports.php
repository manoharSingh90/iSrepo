...
class Reports extends Admin_Controller {
	...
	public function index()
	{
       ...
       $Campaigns = $this->report_model->getTotalViewed(CAMPAIGNS,'campaign_viewed',array('is_active'=>1,"created_dttm >="=>$startDt,"created_dttm <="=>$endDt));
		...
		for($i=0;$i<sizeof($day);$i++){
                	if($SampleRedeemed[$i]['Days'] =='Sunday') {						
                		$tempArr = array_merge($tempArr,$SampleRedeemed[$i]);
                		$srArr[$i] = $tempArr;
                		$tempArr = array();
		...
		foreach ($day as $Dkey=>$value ) {
			...
			//$uagSample = $uagSample==''?0:$uagSample;
			...
			//$uagReview = $uagReview==''?0:$uagReview;
			...
			//$uagComment = $uagComment==''?0:$uagComment;
			if(!empty($Likes))
			{
			...
			//$uagCamp = $uagCamp==''?0:$uagCamp;

			array_push($data['usages_trands'], array($day[$Dkey],$uagSample,$uagPromo,$uagReview,$uagComment,$uagLikes,$uagBrand,$uagCamp));
		}
		//$uagSample1 = array_map($this->myfunction(),$data['usages_trands']);
		//echo"<pre>";print_r($uagSample1);die;
	...
	public function convertNullToZero($response){
	...
	public function excelExport()
	{
		...
		$sheet->setCellValue('D' . $rows, "".sprintf('%0.2f', $malePercent,2)."");
		$sheet->setCellValue('E' . $rows, "".sprintf('%0.2f', $femalePercent,2)."");
        ...
        if(!empty($profileAge)) {
        	foreach ($profileAge as $key => $ageValue) {
        		...
        		$sheet->setCellValue('D' . $rows, "".sprintf('%0.2f', $ageValue->Male_percent,2)."");
        		$sheet->setCellValue('E' . $rows, $ageValue->Female);            
        		$sheet->setCellValue('F' . $rows, "". sprintf('%0.2f',$ageValue->Female_percent,2)."");
        ...
        if(!empty($profileInterest)) {
        	foreach ($profileInterest as $key => $intrestValue) {
        		...
        		$sheet->setCellValue('D' . $rows, "".sprintf('%0.2f', $intrestValue->Male_percent,2)."");
        		$sheet->setCellValue('E' . $rows, $intrestValue->Female);            
        		$sheet->setCellValue('F' . $rows, "".sprintf('%0.2f', $intrestValue->Female_percent,2)."");            
        		$rows++;
        	}
        }
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        ...
        if(!empty($profileCountry)) {
        	foreach ($profileCountry as $key => $countryValue) {
        		...
        		$sheet->setCellValue('D' . $rows, "".sprintf('%0.2f', $countryValue->Male_percent,2)."");
        		$sheet->setCellValue('E' . $rows, $countryValue->Female);            
        		$sheet->setCellValue('F' . $rows, "".sprintf('%0.2f', $countryValue->Female_percent,2)."");            
        		$rows++;
        	}
        }
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        ...
        $profileCompArr = $this->report_model->getProfileCompletion(array("u.is_active"=>1,"u.registration_status"=>2,"u.gender !="=>0,"u.created_dttm >="=>$startDt,"u.created_dttm <="=>$endDt));
        $profileArr = [];		
        ...
        foreach ($finalArray as $key => $dataValue) {			
        	$G = $dataValue["Male"];
        	$B = $dataValue["Female"];			
        	array_push($excel['profileChart'], array('"'.$dataValue['ProfileCompletion'].'%"',(int)$G,(int)$B));
       ...
       $Campaigns = $this->report_model->getTotalViewed(CAMPAIGNS,'campaign_viewed',array('is_active'=>1,"created_dttm >="=>$startDt,"created_dttm <="=>$endDt));
        ...
        $usagesTrands[] = array("Days", "Sample Redeemed", "Promos Redeemed", "Reviews Given", "Comments", "likes","Brand Page Viewed","Campaigns Viewed");
        $day = array('Sunday'=>'Sun','Monday'=>'Mon','Tuesday'=>'Tue','Wednesday'=>'Wed','Thursday'=>'Thu','Friday'=>'Fri','Saturday'=>'Sat');
        foreach ($day as $Dkey=>$value ) {
		...
		$sheet->getStyle('A1:H1' )->applyFromArray($styleArrayFirstRow);
        $rows= 1;
        if(!empty($usagesTrands)) {
        	...
        	for ($ut=0; $ut<$countUt; $ut++) {
        		...
        		$sheet->setCellValue('H' . $rows, str_replace('"',' ',$usagesTrands[$ut][7]));
    ...
    public function campaignDetailExcelExport(){
		...
		$sheet->setCellValue('A1', 'Post Description');
		...
		if(!empty($promoActivity)) {
			foreach ($promoActivity as $key => $promoValue) {
				$postType = $promoValue['banner_type']==1?'Text + Image':'Text + Video';
				$promoDate = date('d M, Y',strtotime($promoValue['publish_date']));
				$totalNps = NPS($promoValue['id']);
				$sheet->setCellValue('A' . $rows, $promoValue['post_desc']);
		...
		if(!empty($campaignsPromoLike)) {
			foreach ($campaignsPromoLike as $key => $likeValue) {				
				$postType = $likeValue['banner_type']==1?'Text + Image':'Text + Video';
				$sheet->setCellValue('A' . $rows, $likeValue['post_desc']);
				$sheet->setCellValue('B' . $rows, $postType);
				$sheet->setCellValue('C' . $rows, $likeValue['no_of_likes']);
				$sheet->setCellValue('D' . $rows, $likeValue['MALE']);
				$sheet->setCellValue('E' . $rows, "".sprintf('%0.2f', $likeValue['M_percent'],2)."");
				$sheet->setCellValue('F' . $rows, $likeValue['Female']);
				$sheet->setCellValue('G' . $rows, "".sprintf('%0.2f', $likeValue['F_percent'],2)."");
				$rows++;
		...
		if(!empty($campaignsPromoComment)) {
			foreach ($campaignsPromoComment as $key => $commentValue) {				
				$postType = $commentValue['banner_type']==1?'Text + Image':'Text + Video';
				$sheet->setCellValue('A' . $rows, $commentValue['post_desc']);
				$sheet->setCellValue('B' . $rows, $postType);		           
				$sheet->setCellValue('C' . $rows, $commentValue['no_of_comments']);
				$sheet->setCellValue('D' . $rows, $commentValue['MALE']);
				$sheet->setCellValue('E' . $rows, "".sprintf('%0.2f', $commentValue['M_percent'],2)."");
				$sheet->setCellValue('F' . $rows, $commentValue['Female']);
				$sheet->setCellValue('G' . $rows, "".sprintf('%0.2f', $commentValue['F_percent'],2)."");
				$rows++;
		...
		if(!empty($campaignsRegularPost)) {
			foreach ($campaignsRegularPost as $key => $regValue) {				
				...
				$sheet->setCellValue('A' . $rows, $regValue['post_desc']);
				$sheet->setCellValue('B' . $rows, $regValue['no_of_likes']);
				$sheet->setCellValue('C' . $rows, $maleLike);
				$sheet->setCellValue('D' . $rows, "".sprintf('%0.2f', $maleLike_per,2)."");
				$sheet->setCellValue('E' . $rows, $femaleLike);
				$sheet->setCellValue('F' . $rows, "".sprintf('%0.2f', $femaleLike_per,2)."");
				$sheet->setCellValue('G' . $rows, $regValue['no_of_comments']);
				$sheet->setCellValue('H' . $rows, $maleCmnt);
				$sheet->setCellValue('I' . $rows, "".sprintf('%0.2f', $maleCmnt_per,2)."");
				$sheet->setCellValue('J' . $rows, $femaleCmnt);
				$sheet->setCellValue('K' . $rows, "".sprintf('%0.2f', $femaleCmnt_per,2)."");
				$rows++;
	...
	public function campaigns()
	{
		...
		//	LIVE CAMPAIGNS CODE
		if($campaignData['range'] =='-'){
			$campaignData['start'] = date('d M Y',strtotime($startDt));
			$campaignData['end'] = date('d M Y',strtotime($endDt));
		}else{
		...
		//echo"<pre>";print_r(groupArray($reviews_campaign, "question_id", true));die;
		...
		$campaignData['reviewAns'] = $reviews_campaign;
	...
	public function campaignExcelExport()
	{
		...
		if(!empty($campaignsReviewQus)) {
			foreach ($campaignsReviewQus as $key => $campaignValue) {
				...
				$sheet->setCellValue('C' . $rows, "".sprintf('%0.2f', $campaignValue['M_percent'],2)."");
				$sheet->setCellValue('D' . $rows, $campaignValue['Female']);
				$sheet->setCellValue('E' . $rows, "".sprintf('%0.2f', $campaignValue['F_percent'],2)."");
				$rows++;
	...
	public function vendingMachine()
	{
		...
		$startDt = isset($startDt)==''?date('Y-m-d 00:00:00'):$startDt;
		$endDt = isset($endDt)==''?date('Y-m-d 23:59:59'):$endDt;		
		if($machineData['range'] =='-'){
			$machineData['start'] = date('d M Y',strtotime($startDt));
			$machineData['end'] = date('d M Y',strtotime($endDt));
		}else{
	...
	function campaignCompletionReport($campaign_id)
	{
		require(APPPATH . 'third_party/PhpSpreadsheet/vendor/autoload.php');
		$fileName = "campaign_completion_report.xlsx";
		$filePath = 'assets/files/';
		//print_r($filePath.$fileName);die;
		$spreadsheet = new Spreadsheet();
		$spreadsheet->createSheet(0);
		$campDtl = $this->report_model->getCampaignDtl(array("cmp.id"=>$campaign_id));
		$nps =  NPS($campDtl['id']);
        //$totalSamples = $campDtl['vend_no_of_sample_used'] + $campDtl['vend_no_of_samples'];
        $totalSamples = $campDtl['vend_no_of_samples'];
        
        $cond= "ur.review_id ='".$campDtl['review_id']." ' AND ur.review_text !='' AND ur.review_text IS NOT NULL";
        $openEndedReviews = $this->report_model->getOpenEndedReview($cond);
        $ratingsNo = $this->report_model->getOpenEndedReview(array("ur.review_id"=>$campDtl['review_id']));

        $userCount = $this->report_model->getUserReviewCount(array("ura.review_id"=>$campDtl['review_id']));
        $userProfileAge = $this->report_model->getUserCampaignProfile(array("cmp.id"=>$campaign_id));
        $userProfileGen= $this->report_model->getUserGender(array("cmp.id"=>$campaign_id));

        $smsCondArr = "cmp.id ='".$campaign_id."' AND u.smsqr_code_url IS NOT NULL AND u.smsqr_code_url <> '' ";
        $smsUsers= $this->report_model->numUserCount($smsCondArr);
        $ChatCond = "cmp.id ='".$campaign_id."' AND u.fb_id IS NOT NULL AND u.fb_id <> '' ";
        $chatbotUsers= $this->report_model->numUserCount($ChatCond);
        $AppCond = "cmp.id ='".$campaign_id."' AND u.fb_id IS NULL AND u.imei IS NULL ";
        $appUsers= $this->report_model->numUserCount($AppCond);
        
        $mobCond = "cmp.id ='".$campaign_id."' AND u.phone IS NOT NULL AND u.phone <> '' ";
        $mobileCount= $this->report_model->numUserCount($mobCond);
        $emailCond = "cmp.id ='".$campaign_id."' AND u.email IS NOT NULL AND u.email <> '' ";
        $emailCount= $this->report_model->numUserCount($emailCond);
        $fbCond = "cmp.id ='".$campaign_id."' AND u.fb_id IS NOT NULL AND u.fb_id <> '' ";
        $fbCount= $this->report_model->numUserCount($fbCond);

        $ageGroupRating = $this->report_model->getAgeGroupRating(array("cmp.id"=>$campaign_id));

        $feedbackQus = $this->report_model->getFeedbackQus(array("rq.review_id"=>$campDtl['review_id']));
        //echo"<pre>";print_r($feedbackQus);die;
		$sheet = $spreadsheet->setActiveSheetIndex(0);
		$sheet->setTitle("CAMPAIGN COMPLETION REPORT");
		$styleArrayFirstRow = [
		            'font' => [
		                'bold' => true,
		            ]
		...
		$styleCenterArray = array(
      	...
      	$styleArray = array('alignment' => [
		    'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
		    'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
		]);
		$sheet->getStyle('A1:C1' )->applyFromArray($styleArray);
				
		$sheet->setCellValue('A1', 'CAMPAIGN COMPLETION REPORT');
            $sheet->setCellValue('A2', "".'Campaign name ( '.$campDtl['campaign_name'].' )'."");
            $sheet->setCellValue('B2', "".'Start Date ('.date('d M Y',strtotime($campDtl['start_date'])).' )'."");
            $sheet->setCellValue('C2', "".'End Date ('.date('d M Y',strtotime($campDtl['end_date'])).' )'."");
            $sheet->setCellValue('A4', "".'Date of Completion'."");
            $sheet->setCellValue('B4', "".date('d M Y',strtotime($campDtl['end_date']))."");
            $sheet->setCellValue('A5', "".'Total number of samples'."");
            $sheet->setCellValue('B5', "".$totalSamples."");
            $sheet->setCellValue('A6', "".'Number of samples dispensed '."");
            $sheet->setCellValue('B6', "".$campDtl['vend_no_of_sample_used']."");
            $sheet->setCellValue('A7', "".'Number of ratings'."");
            $sheet->setCellValue('B7', "".$ratingsNo['review_count']."");
            $sheet->setCellValue('A8', "".'Average rating'."");
            $sheet->setCellValue('B8', "".$campDtl['avg_rating']."");
            $sheet->setCellValue('A9', "".'Number of open ended reviews '."");
            $sheet->setCellValue('B9', "".$openEndedReviews['review_count']."");
            $sheet->setCellValue('A10', "".'Users who answered feedback questions'."");
            $sheet->setCellValue('B10', "".$userCount['usercount']."");
            $sheet->setCellValue('A11', "".'Net Promoter score'."");
            $sheet->setCellValue('B11', "".$nps."");
            $sheet->setCellValue('A12', "".'Clicks on BUY NOW'."");
            $sheet->setCellValue('B12', "".$campDtl['buy_now_click_total']."");
            
            $sheet->getStyle('A14:C14' )->applyFromArray($styleArrayFirstRow);
            $spreadsheet->getActiveSheet()->mergeCells("A14:C14");
            $sheet->setCellValue('A14', 'USER PROFILE');
            $sheet->getStyle('A14:C14' )->applyFromArray($styleArray);

            $sheet->setCellValue('A15', "".'Male'."");
            $sheet->setCellValue('B15', "".sprintf('%0.2f', $userProfileGen['M_percent'],2).'%'."");
            $sheet->setCellValue('A16', "".'Female'."");
            $sheet->setCellValue('B16', "".sprintf('%0.2f', $userProfileGen['F_percent'],2).'%'."");

            $sheet->getStyle('A18:C18' )->applyFromArray($styleArrayFirstRow);
            $spreadsheet->getActiveSheet()->mergeCells("A18:C18");
            $sheet->setCellValue('A18', 'Age Group');
            $sheet->getStyle('A18:C18' )->applyFromArray($styleArray);
            $sheet->setCellValue('A19', "".'Age Group'."");
            $sheet->setCellValue('B19', "".'Male'."");
            $sheet->setCellValue('C19', "".'Female'."");
            $row=20;
            foreach ($userProfileAge as $agekey => $agevalue) {
            	$sheet->setCellValue('A' . $row , "".$agevalue['bracket']."");
            	$sheet->setCellValue('B' . $row , "".sprintf('%0.2f', $agevalue['M_percent'],2).'%'."");
            	$sheet->setCellValue('C' . $row , "".sprintf('%0.2f', $agevalue['F_percent'],2).'%'."");
            ...
            $sheet->getStyle('A'.$row.':C'.$row.'' )->applyFromArray($styleArrayFirstRow);
            $spreadsheet->getActiveSheet()->mergeCells("A".$row.":C".$row);
            $sheet->setCellValue('A'.$row , 'USER JOURNEY VIA');
            $sheet->getStyle('A'.$row.':C'.$row )->applyFromArray($styleArray);
            $row++;

            $sheet->setCellValue('A'.$row , 'SMS');
            $sheet->setCellValue('B'.$row , 'APP');
            $sheet->setCellValue('C'.$row , 'Messenger');
            $row++;
			$sheet->setCellValue('A'.$row , "".$smsUsers['num_user']."");
            $sheet->setCellValue('B'.$row , "".$appUsers['num_user']."");
            $sheet->setCellValue('C'.$row , "".$chatbotUsers['num_user']."");
            $row++;
            $sheet->setCellValue('A'.$row , "".'Number of mobile phones'."");
            $sheet->setCellValue('B'.$row , "".'Number of email ids'."");
            $sheet->setCellValue('C'.$row , "".'Number of facebook ids'."");
            $row++;
            $sheet->setCellValue('A'.$row , "".$mobileCount['num_user']."");
            $sheet->setCellValue('B'.$row , "".$emailCount['num_user']."");
            $sheet->setCellValue('C'.$row , "".$fbCount['num_user']."");
            $row++;
            ...
            $sheet->getStyle('A'.$row.':C'.$row )->applyFromArray($styleArrayFirstRow);
            $spreadsheet->getActiveSheet()->mergeCells("A".$row.":C".$row);
            $sheet->setCellValue('A'.$row , "".'Ratings Via Age group'."");            
            $sheet->getStyle('A'.$row.':C'.$row )->applyFromArray($styleArray);
            $row++;
            $sheet->setCellValue('A'. $row , "".'Age Group'."");
            $sheet->setCellValue('B'. $row , "".'Avg Ratings'."");
            $row++;
            foreach ($ageGroupRating as $groupkey => $groupvalue) {
            	$sheet->setCellValue('A' . $row , "".$groupvalue['bracket']."");
            	$sheet->setCellValue('B' . $row , "".sprintf('%0.2f', $groupvalue['avg_rat'],2)."");
            	//$sheet->setCellValue('C' . $row , "".sprintf('%0.2f', $agevalue['F_percent'],2).'%'."");
            	$row++;
            }
            ...
            $sheet->getStyle('A'.$row.':C'.$row )->applyFromArray($styleArrayFirstRow);
            $spreadsheet->getActiveSheet()->mergeCells("A".$row.":C".$row);
            $sheet->setCellValue('A'.$row , "".'Feedback Questions'."");            
            $sheet->getStyle('A'.$row.':C'.$row )->applyFromArray($styleArray);
            $row++;

            $sheet->setCellValue('A'. $row , "".'Question'."");
            $sheet->setCellValue('B'. $row , "".'Answer'."");
            $sheet->setCellValue('C'. $row , "".'Male Count'."");
            $sheet->setCellValue('D'. $row , "".'Female Count'."");
            $sheet->setCellValue('E'. $row , "".'Age Group'."");
            $row++;

            foreach ($feedbackQus as $quskey => $qusvalue) {
            	
            	$sheet->setCellValue('A' . $row , "".$qusvalue['ques_text']."");
            	foreach ($qusvalue['review_ans'] as $anskey => $ansvalue) {
            		$mcount = $this->report_model->getQuesDistribution(array('ura.answer_id'=>$ansvalue['id'],"u.gender"=>1));
            		$fcount = $this->report_model->getQuesDistribution(array('ura.answer_id'=>$ansvalue['id'],"u.gender"=>2));
            		$ageGr = $this->report_model->getQuesDistributionAge(array('ura.answer_id'=>$ansvalue['id']));
            		$sheet->setCellValue('B' . $row , "".$ansvalue['answer_text']."");
            		$sheet->setCellValue('C' . $row , "".$mcount['tCount']."");
            		$sheet->setCellValue('D' . $row , "".$fcount['tCount']."");
            		$sheet->setCellValue('E' . $row , "".$ageGr['ageGroup']."");
            		$row++;
            	}
            	$row++;
            }
