...
class Mobile extends CI_Controller {
	...
	function register(){
		...
		if(!empty($this->data))
		{
			...
			if($social_login_id==''){
				...
				$users=$this->api_model->getRowData(USERS,'email,phone',$con);
				if(!empty($users)){
					if($users->phone==$phone)
						$msg['message']='Phone number already exists.';
					else if($users->email==$email)
						$msg['message']='Email address already exists.';
					$msg['status']='error'; 
					$msg['status_code']=300;
				...
				$con= "social_oath_token ='".$social_login_id."'";
	...
	function otp_verification(){
		...
		{	
			...
			if($user_id=='' || $otp==''){
				...
				else if($users->registration_status==1){
					$userData = array(
						'registration_status'=> '2' ,
						'modified_dttm'  	=> date('Y-m-d H:i:s'),
					);
				...
				else
				{
					$response['status']=false; 
					$response['status_code']=300;
					$response['message']='OTP is already verified.';
				}
			}
		}
		else
	...
	function login(){  
		...
		{	
			...
			if($email=='' || $password==''){
				...
				$users     = $this->api_model->getRowData(USERS,'id as user_id,name,email,phone,age_bracket_id as age,gender,image,registration_status,',array('email' => $email,'password' =>  md5(trim($password))));
	...
	function campaign_list(){
		...
		if(!empty($this->data))
		{
			$user_id='';$campaign_id='';$campaign_status='';$primary_image='';
			if(isset($this->data['user_id']))
				$user_id=$this->data['user_id'];
			...
			if($user_id==''){
				...
				if($campaign_id)
					$conditions .= " AND  camp.id LIKE '".$campaign_id."'";
				
				if($campaign_status==1)
				{
					$conditions .= " AND STR_TO_DATE(camp.start_date, '%Y-%m-%d') <='".date('Y-m-d')."' AND STR_TO_DATE(camp.end_date, '%Y-%m-%d') >='".date('Y-m-d')."'";
				}
				else if($campaign_status==2)
				{
					$conditions .= " AND STR_TO_DATE(camp.start_date, '%Y-%m-%d') >'".date('Y-m-d')."'";
				}
				else if($campaign_status==3)
				{
					$conditions .= " AND STR_TO_DATE(camp.start_date, '%Y-%m-%d') <'".date('Y-m-d',strtotime(date('Y-m-d')))."' AND STR_TO_DATE(camp.end_date, '%Y-%m-%d') <'".date('Y-m-d')."'";
				}
				...
				if($campaign){
					foreach ($campaign as $value) {
						$campaignBanners=$this->api_model->getResultData(CAMPAIGNBANNERS,'id,banner_type,banner_url',array('campaign_id'=>$value->id,'is_active'=>'1'));
						...
						else if (date('Y-m-d',strtotime($value->start_date)) > date('Y-m-d')) 
							$campaign_status='2';
						else if(date('Y-m-d',strtotime($value->end_date)) < date('Y-m-d'))
							$campaign_status='3';
						$requested=false;
	...
	function campaign_review(){
		...
		if(!empty($this->data))
		{
			$limit=10; $offset=0; $campaign_id=''; $photo_url='';
		...
		if(!empty($this->data))
		{
			...
			if($campaign_id=='' || $latitude=='' || $longitude==''){
				...
				$newarray	= array(
					...
					"location_address"			=> str_replace('—', '-', $value->location_address),
	...
	public function contact_us(){
		...
		if(!empty($this->data)){ 
			...
			if($user_id!='' && $user_name!='' && $user_email!='' && $comment!=''){
				...
				$data['name']='Admin';
				$data['message']='A new contact message from iSamplez.<br/>Name : '.$user_name.' <br/>Email : '.$user_email.' <br/>message :'.$comment.' ';
				$message=$this->load->view('emailer/emailer_normal',$data,true);
				$mailResponse=sendMail(ADMINMAIL,$subject,$message);
				if($mailResponse)
		        {   
		        	$response['status']=true;
			        $response['status_code']=200;
	...
	public function validateQRCode(){
		...
		if(!empty($data))
		{
			$sample=$this->api_model->getRowData(USER_SAMPLES,'id',array('qr_code'=> $data['sample_QR_code'],'is_active'=> '1','status'=> '1'));
			if(@count($sample)>0){
				...
				$response['status']=false; 
				$response['status_code']=300;
				$response['message']='Invalid Sample QR Code';
			}
		}
		else
		{
	...
	public function updateInventoryStatus(){
		...
		if(!empty($data))
		{
			...
			if($vending_machine_id=='' || $campaign_id=='' || $total_sample==''){
				$response['status']=false; 
				$response['status_code']=300;
				$response['message']='Vending Machine Id,Campaign Id And Total Sample Available are required.';
			}
			else{
				...
				if(@count($stockDtl)>0){
					...
					$campvendData=array(
							    	"campaign_id" 			=>	$campaign_id,
							    	"vend_machine_id" 		=>	$vend_machine_id,
		...
		if(!empty($data))
		{
			...
			if($data['user_id']!='' ){
			  $brandLogoUrl=base_url('assets/brand/logo/'); 
				...
				$notifications = $this->api_model->notificationList('noti.*,b.id as brand_id,b.brand_logo_url,b.brand_name,b.is_active as brand_is_active',$conditions,$params);
				...
				if($notifications){
					foreach ($notifications as $value) {
						$newarray	= array(
							...
							"brand_is_active" 		=> $value->brand_is_active
				...
				$totalUnreadNotification = $this->api_model->mysqlNumRows(NOTIFICATIONS,"id",array('user_to_id'=>$data['user_id'],'is_view'=>'0'));
				$response['status']=true; 
