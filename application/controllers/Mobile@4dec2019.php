...
class Mobile extends CI_Controller {
	...
	function change_phone_number()
	{
		...
		if(!empty($this->data))
		{
				...
				$users=$this->api_model->getRowData(USERS,'phone',$con);
				if(empty($users)){
	...
	function campaign_list(){
		...
		if(!empty($this->data))
		{
			...
			if($user_id==''){
				...
				{
					$conditions .= " AND ((STR_TO_DATE(camp.start_date, '%Y-%m-%d') <'".date('Y-m-d',strtotime(date('Y-m-d')))."' AND STR_TO_DATE(camp.end_date, '%Y-%m-%d') <'".date('Y-m-d')."') OR (STR_TO_DATE(campsample.start_date, '%Y-%m-%d') <'".date('Y-m-d',strtotime(date('Y-m-d')))."' AND STR_TO_DATE(campsample.end_date, '%Y-%m-%d') <'".date('Y-m-d')."')) ";
				}
	...
	public function validateQRCode(){
		...
		if(!empty($data))
		{
			...
			if(@count($sample)>0){
				...
				$device_token = $this->api_model->getField(USERS_DEVICE_DTL,'device_token',array('user_id'=>$sample->user_id));
			 	if($sample->status=="1"){
					...
					$msg="Congrats. QR code sucessfully verified.";
					$response['status']=true; 
					$response['status_code']=200;
			...
			else{//file_put_contents('/var/www/html/log.txt',"Kinesis dev Process mchin3 ".json_encode($data).' '.@$verify_token."\n",FILE_APPEND | LOCK_EX);
				$response['status']=false; 
				$response['status_code']=300;
