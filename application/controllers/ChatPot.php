...
class ChatPot extends CI_Controller {
	...
	{$user_id=79;$sample_id=18;
		...
		if ($url!='') {
			...
			////////////// ADDED BY SAHIL //////////////
			// GET CAMPAIGN SAMPLES
			$campaignSamplesData = $this->api_model->getSamplesCampaignData($sample_id, $campaign_date->campaign_id);
			$campaignSamplesData = !empty($campaignSamplesData) ? json_decode(json_encode($campaignSamplesData), True) : array();
			$data['campaignSamplesData'] = $campaignSamplesData;
			// GET CAMPAIGN SAMPLES
			
			// GET CAMPAIGN DATA
			$campaignData = $this->api_model->getCampaignData($campaign_date->campaign_id);
			$campaignData = !empty($campaignData) ? json_decode(json_encode($campaignData), True) : array();
			...
			{
				$userReviewData = $this->api_model->getUserReviewData($campaignData[0]["review_id"]);
				$userReviewData = !empty($userReviewData) ? json_decode(json_encode($userReviewData), True) : array();
				$data['userReviewData']=$userReviewData;
			}
			// GET USER REVIEW
			...
			$data['is_email'] = 1;
			$reviewData=array();
			$cond="reviewQuest.is_active='1' AND camp.id='".$campaign_date->campaign_id."'";
			$ReviewQuestionData = $this->api_model->reviewQuestions('reviewQuest.*',$cond);
			...
			$is_review=0;
			$review_id=$CampaignBanner=$this->api_model->getField(CAMPAIGNS,'review_id',array('id'=>$campaign_date->campaign_id));
