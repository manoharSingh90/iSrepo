<div class="pageArea">
  ...
  <div class="pageTabs mt-1 extrawWide">
   <ul>
    ...
    <li><a href="<?php echo base_url('create-samples/'.$brand_id.'/'.$campaign_id);?>"  class="<?php if($this->uri->segment(1)=='create-samples') echo "active";?>">Samples</a></li>
      ...
      <li class="disabled" ><a href="<?php echo base_url('create-campaign-brand/'.$brand_id.'/'.$campaign_id);?>"  class="<?php if($this->uri->segment(1)=='create-campaign-brand') echo "active";?>" >Brand Details</a></li>
    </ul>
  </div>
  <div class="createInfo">
   <?php 
   $attributes = array('id' => 'addCampBrands-form');
   echo form_open('admin/brands/addCampBrands', $attributes);?>
  ...
  <div class="pb-5">
    <ul class="galleryImages mt-0">
      <?php if($media) { 
        $count=0;
        foreach ($media as $key => $value) {  ?> 
          <li>
            <?php if($value->asset_type=='1') { 
              if(file_exists('assets/brand/assets/'.$value->asset_url)){
              ?>
            ...
            <?php } } else if($value->asset_type=='2') { 
              if(file_exists('assets/brand/assets/'.$value->asset_url)){
              ?>
            ...
            <div class="custom-control custom-checkbox">
              ...
              <input type="checkbox" class="custom-control-input" id="customCheck-<?php echo $count;?>" >
            ...
            <!--   <input type="text" name="button_type" value="" class=""> -->
          </li>
          <?php $count++;} } 
...
<script type="text/javascript">
  (function($) {
    ...
    $("#button_type").val(type);
     $("#addCampBrands-form").validate({
      errorElement: 'small',
      submitHandler: function() {
        var numberOfChecked = $('input:checkbox:checked').length;
        var countOfMedia="<?php if($media!='') echo count($media); else echo "0";?>";
        if(numberOfChecked<1 && countOfMedia>0){
          $('#ErrorModal').modal();
          return false;
        }
       // $("#create,#publish").attr("disabled", true);
        $("#"+type).attr("disabled", true).addClass("loading");
        $('#startConfetti').trigger('click');
        var user = $('#addCampBrands-form').serialize();
        ...
        $.ajax({  
          type: "POST",  
          url:  "<?php echo base_url(); ?>" + "admin/brands/addCampBrands",  
          data: user, 
          cache: false,  
          success: function(campaign_id){  
          //$("#create,#publish").attr("disabled", false);
...
$(document).on('click', '.removePic', function() {
  ...
  var $itemUpload = $(this).closest('li');
  var uploadData = '<li> <div class="boxImg"><p class="text-center text-uppercase font-weight-bold text-link uploadProfile">+ Upload file</p> </div> <input type="file" class="uploadPic" id="logo" data-width="100" data-height="100" data-img-size="1024"  accept="image/png, image/jpeg, image/jpg"><input type="text" class="hidden" value="" name="logo" /> <div class="removePic"><b>X</b> File: </div> </li>'
